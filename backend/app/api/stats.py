...
async def get_summary(db: AsyncSession = Depends(get_db)) -> Dict[str, int]:
    ...
    result = await db.execute(
        ...
        .where(TeacherModel.is_active == True)
    )
    ...
    result = await db.execute(
        ...
        .where(ClassModel.status == "active")
    )
...
async def get_dashboard_stats(db: AsyncSession = Depends(get_db)) -> Dict[str, Any]:
    ...
    try:
        result = await db.execute(
            select(StudentModel.payment_cluster, func.count(StudentModel.id))
            .where(StudentModel.is_active == True)
    ...
    try:
        result = await db.execute(
            select(ClassModel.status, func.count(ClassModel.id)).group_by(
                ClassModel.status
            )
        )
    ...
    try:
        result = await db.execute(
            select(TeacherModel.full_name, func.count(ClassModel.id))
            .outerjoin(ClassModel, ClassModel.teacher_id == TeacherModel.id)
            .where(TeacherModel.is_active == True)
            .group_by(TeacherModel.id, TeacherModel.full_name)
            .order_by(TeacherModel.full_name)
        )
        teacher_class_count = [
            {"teacher_name": row[0], "class_count": row[1]} for row in result.all()
        ]
    except Exception as e:
        print(f"Error getting teacher class counts: {e}")
        teacher_class_count = []
