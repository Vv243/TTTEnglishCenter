...
export default function AddEnrollmentModal({ isOpen, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    ...
    classesAPI.getAll({ per_page: 100 }).then((r) =>
      setClasses(r.items.filter((c) => c.status === "active" || c.status === "scheduled"))
    );
  }, [isOpen]);
  ...
  const classIsFull = selectedClass
    ? selectedClass.current_enrollment >= selectedClass.max_students
    : false;
    ...
    try {
      ...
      setError(
        typeof detail === "string"
          ? detail
          : "Failed to create enrollment. The student may already be enrolled in this class."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-amber-50 to-white">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-0.5">
              Enroll a student into an active or scheduled class
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-5 space-y-5 max-h-[72vh] overflow-y-auto">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 capitalize">
                      {selectedStudent.grade_level?.replace(/_/g, " ")}
                    </p>
                  </div>
                </div>
                <button
                  onClick={() => { set("student_id", ""); setStudentSearch(""); }}
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 capitalize">
                            {s.grade_level?.replace(/_/g, " ")} · {s.parent_name}
                          </p>
                        </div>
                      </button>
                    ))
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between px-3 py-2 bg-blue-50 border border-blue-200 rounded-lg">
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-mono">
                      {selectedClass.class_code} · {selectedClass.current_enrollment}/
                      {selectedClass.max_students} students
                    </p>
                  </div>
                  <button
                    onClick={() => { set("class_id", ""); setClassSearch(""); }}
                    className="text-slate-400 hover:text-red-500"
                  >
                    <X className="h-4 w-4" />
                  </button>
                ...
                {classIsFull && (
                  <div className="px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700">
                    ⚠️ This class is full ({selectedClass.current_enrollment}/
                    {selectedClass.max_students}). Student will be added to the waitlist.
                  </div>
                )}
              </div>
            ) : (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs text-slate-400 font-mono">
                            {c.class_code} · {c.level.replace(/_/g, " ")}
                          </p>
                        </div>
                        <span
          ...
          <div className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-500">
            {classIsFull ? (
              <>Student will be placed on the <strong>waitlist</strong>. Teacher must approve before payment.</>
            ) : (
              <>Student will be set to <strong>Pending</strong>. Status auto-updates to <strong>Enrolled</strong> once payment is recorded.</>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50">
          ...
          >
            {loading ? "Enrolling..." : classIsFull ? "Join Waitlist" : "Enroll Student"}
          </Button>
        </div>
      </div>
    </div>
