    ...
    return (
      <div className="flex items-center justify-center h-screen bg-slate-900">
        <div className="text-center">
          ...
          <p className="text-slate-400 font-mono text-sm">Loading payment intelligence...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-start justify-between">
          <div>
            <div className="flex items-center gap-3 mb-1">
              ...
              <h1 className="text-2xl font-bold text-white tracking-tight">Payment Intelligence</h1>
            </div>
            <p className="text-slate-400 text-sm ml-11">
              90-day revenue forecast · Risk analysis · {s.active_students} active students
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-slate-500 font-mono mb-1">FORECAST METHOD</p>
            <p className="text-xs text-amber-400/80">Rule-based × Prophet Trend</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-8 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <KPICard
            ...
            value={formatVNDShort(s.expected_next_month)}
            sub={`avg historical: ${formatVNDShort(s.avg_historical_monthly)}`}
            accent="amber"
          />
          <KPICard
            label="90-Day Forecast"
            value={formatVNDShort(s.total_expected_90_days)}
            sub="March – May 2026"
            accent="amber"
          />
          <KPICard
            label="Collection Rate"
            value={`${(r.expected_collection_rate * 100).toFixed(0)}%`}
            sub={`${r.high_risk_count} high-risk students`}
            accent={r.expected_collection_rate >= 0.85 ? "green" : r.expected_collection_rate >= 0.70 ? "amber" : "red"}
          />
          <KPICard
            label="Trend Multiplier"
            value={`${s.trend_multiplier > 1 ? "+" : ""}${((s.trend_multiplier - 1) * 100).toFixed(1)}%`}
            sub="Prophet growth signal"
            accent={s.trend_multiplier >= 1 ? "green" : "red"}
          />
        </div>
        ...
        <div className="flex gap-1 bg-slate-800/50 rounded-lg p-1 w-fit">
          {(["forecast", "risk"] as const).map((tab) => (
            ...
            >
              {tab === "forecast" ? "📈 Revenue Forecast" : "⚠️ Payment Risk"}
            </button>
          ))}
        </div>
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-4 text-xs">
                  ...
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 bg-blue-400 border-dashed inline-block" style={{ borderTop: "2px dashed #60a5fa", background: "none" }} />
                  ...
                  {forecast!.forecast.map((m, i) => (
                    <tr key={m.month} className={`border-t border-slate-700/30 ${i === 0 ? "bg-amber-500/5" : ""}`}>
                      ...
                      <td className="px-6 py-4 text-right font-mono text-sm text-slate-500">
                        ±{formatVNDShort(m.expected_revenue - m.lower_bound)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="bg-slate-800/40 border border-slate-700/50 rounded-xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="font-mono text-sm text-slate-400">
                  Expected: <span className="text-white">{formatVNDShort(r.total_expected_vnd)}</span>
                  <span className="text-slate-500"> / </span>
                  <span className="text-slate-400">{formatVNDShort(r.total_billed_vnd)} billed</span>
                </span>
              </div>

              {/* Risk distribution bar */}
              <div className="flex rounded-full overflow-hidden h-3 mb-3">
                <div className="bg-red-500 transition-all" style={{ width: `${(r.high_risk_count / r.total_students) * 100}%` }} />
                <div className="bg-amber-400 transition-all" style={{ width: `${(r.medium_risk_count / r.total_students) * 100}%` }} />
                <div className="bg-emerald-500 transition-all" style={{ width: `${(r.low_risk_count / r.total_students) * 100}%` }} />
              </div>
              <div className="flex gap-6 text-xs">
                <span className="flex items-center gap-1.5 text-red-400"><span className="w-2 h-2 rounded-full bg-red-500" />{r.high_risk_count} High Risk</span>
                <span className="flex items-center gap-1.5 text-amber-400"><span className="w-2 h-2 rounded-full bg-amber-400" />{r.medium_risk_count} Medium</span>
                <span className="flex items-center gap-1.5 text-emerald-400"><span className="w-2 h-2 rounded-full bg-emerald-500" />{r.low_risk_count} Low Risk</span>
              </div>
            </div>
                      ...
                      <tr key={s.student_id} className="border-t border-slate-700/30 hover:bg-slate-700/20 transition-colors">
                        ...
                        <td className="px-6 py-3 text-center">
                          <span
                            ...
                            style={{
                              backgroundColor: `${RISK_COLORS[s.risk_level]}20`,
